...
# -------------------------------------------------------------
# The exact feature names requested by your model's training data schema
EXPECTED_FEATURES = [
...
def predict():
    ...
    try:
        # Build dictionary matching values
        input_data = {}
        for feature in EXPECTED_FEATURES:
            ...
            # Numeric type formatting matching training distribution arrays
            if feature in ['Age', 'Billing Amount']:
                input_data[feature] = float(val)
            else:
                # String/Categorical properties.
                # NOTE: If your model requires raw Label Encoded/One-Hot numeric inputs instead of raw strings,
                # this converts directly to string category type which native XGBoost versions support automatically.
                input_data[feature] = str(val)

        # Convert input dictionary into a pandas DataFrame and force the strict feature sequence matching training data.
        input_df = pd.DataFrame([input_data])
        input_df = input_df[EXPECTED_FEATURES]

        # Handle object/string types as internal category types for native XGBoost booster architectures
