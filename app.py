...
# Dropdown list options for categorical metrics to make selection attractive
DROPDOWN_OPTIONS = {
    'Gender': ['Male', 'Female'],
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-lg font-bold tracking-tight text-slate-900">
                        ClinicalPredict <span class="text-xs bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-full font-medium ml-1">XGBoost Framework</span>
                    </h1>
                ...
                <form action="/predict" method="POST" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="flex flex-col space-y-1.5">
                            ...
                            {% else %}
                                <input 
                                    ...
                                    value="{{ user_inputs[feature] if user_inputs else '' }}"
                    ...
                    <div class="sm:col-span-2 pt-4 border-t border-slate-100 mt-4">
                        <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-xl shadow-lg shadow-indigo-600/20 transition transform active:scale-[0.99] flex items-center justify-center space-x-2 cursor-pointer">
                            ...
                            <span>Compute Dynamic Profile Analytics</span>
                        </button>
                    </div>
                </form>
            </div>
                    ...
                    {% if prediction_made %}
                        ...
                        <div class="p-4 rounded-xl text-sm leading-relaxed bg-slate-50 border border-slate-200 text-slate-700">
                            <div class="font-bold text-slate-900 mb-1 flex items-center space-x-1.5">
                                ...
                                <span>Structural Validation Note</span>
                            </div>
                            The patient profile features matched the exact matrices required by your custom healthcare pipeline. Model classification compiled safely.
                        </div>
                    {% else %}
                        <div class="flex flex-col items-center justify-center text-center py-20 px-4 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                            <div class="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 mb-4">
                                <i class="fa-solid fa-server text-2xl animate-pulse"></i>
                            </div>
                            <h4 class="text-sm font-semibold text-slate-700 mb-1">Awaiting Data Execution</h4>
                            <p class="text-xs text-slate-400 max-w-[200px]">Fill out the patient demographic records to resolve real-time model calculations.</p>
                        </div>
                    {% endif %}
                </div>

                <div class="text-[10px] text-slate-400 border-t border-slate-100 pt-4 mt-6 text-center">
                    <i class="fa-solid fa-shield-halved mr-1"></i> Feature-space validation integrity: Active.
                </div>
            </div>
        </div>
...
def predict():
    ...
    try:
        ...
        # Convert input dictionary into a pandas DataFrame matching exact order
        input_df = pd.DataFrame([input_data])
        input_df = input_df[EXPECTED_FEATURES]

        # Crucial Fix: Explicitly cast categorical columns as 'category' dtypes
        for col in input_df.columns:
            if input_df[col].dtype == 'object':
                input_df[col] = input_df[col].astype('category')

        # -------------------------------------------------------------
        # DYNAMIC PREDICTION HANDLER (Solves enable_categorical error)
# -------------------------------------------------------------
        # Try evaluating using native Scikit-Learn wrapper methods with dynamic flags
        try:
            ...
            # Fallback for raw Booster artifacts that require building a core DMatrix container
            dtrain = xgb.DMatrix(input_df, enable_categorical=True)
            raw_pred = model.predict(dtrain)
            
            # Extract argmax classification bounds out of multidimensional probability arrays
            if len(raw_pred.shape) > 1 and raw_pred.shape[1] > 1:
                prediction = int(np.argmax(raw_pred[0]))
                confidence = round(float(np.max(raw_pred[0])) * 100, 2)
            else:
                prediction = int(round(raw_pred[0])) if raw_pred[0].size == 1 else int(round(raw_pred[0][0]))
                confidence = None
